...
{"name":"X","age":28}
```
As you may expect, the compiler tries to find evidence that there exists an implicit instance of `JsonWriter[Persor]' in the context and if it fails to ensure the instance is available, it raises an error and the compile would fail.
